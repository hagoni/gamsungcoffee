...
(function($) {
	...
	window.Sitemap = function() {
		...
		var $topElement = $('body'),
			$container = $('#sitemapWrap'),
			_anchor = '.bindSitemapSpread',
			_btnX = '.bindSitemapFold',
			spread = false,
			tween = false;
		 ...
		 */
		this.initialize = function() {
			$topElement.on('click', _anchor, this.spread);
			$topElement.on('click', _btnX, this.fold);
		};
...
(function($) {
	$(document).ready(function() {
		...
		(function() {
            var headerMotion = [
                [
					{el: $('.hd_num'), to: {text: {value: ''}}, d: 0.001},
		            {method: 'add', tl: [
		                TweenMax.to({number: 0}, 1.0, {number: $('.hd_num1').data('number'), ease: Back.easeOut, onUpdate: function() {
		                    $('.hd_num1').text(parseInt((this.vars['number'] * this.progress()), 10).toLocaleString().split('.')[0]);
		                }, onComplete: function() {
		                    $('.hd_num1').text(parseInt(this.vars['number'], 10).toLocaleString().split('.')[0]);
		                }}),
						TweenMax.to({number: 0}, 1.0, {number: $('.hd_num2').data('number'), ease: Back.easeOut, onUpdate: function() {
		                    $('.hd_num2').text(parseInt((this.vars['number'] * this.progress()), 10).toLocaleString().split('.')[0]);
		                }, onComplete: function() {
		                    $('.hd_num2').text(parseInt(this.vars['number'], 10).toLocaleString().split('.')[0]);
		                }})
		            ]},
                ]
            ];
            new YMotion(headerMotion, {oElems: 'header-offset'}).activate();
        }());
	});
}(jQuery));
...
(function($) {
	...
	Geomap.prototype.initialize = function(container, options, callback) {
        ...
        this.infoWindowOptions = typeof this.opt.infoWindow.options === 'object' ? this.opt.infoWindow.options : {};
		var opt = this.opt;
		var generateMap = function(geocode) {
			if(typeof opt.mapOptions.center === 'undefined') opt.mapOptions.center = new naver.maps.LatLng(geocode.lat, geocode.lng);
			_this.map = new naver.maps.Map(document.getElementById(container), opt.mapOptions);
			_this.map.setOptions('mapTypeControl', true);
	 ...
	 */
	 Geomap.prototype.getCurrentPosition = function(callback, geocode) {
         if(window.location.protocol.toLowerCase().indexOf('https') > -1 && this.checkMobile() === true) {
	...
	Geomap.prototype.setClientLocationMarker = function(geocode) {
		new naver.maps.Marker({
			position: new naver.maps.LatLng(geocode.lat, geocode.lng),
			map: this.map,
			title: '내 위치'
		});
	};
	...
	Geomap.prototype.setPlaceMarkers = function(requestUrl, data, callback) {
		...
		$.post(requestUrl, data, function(response) {
			...
			for(var i=0; i<length; i++) {
				...
				if(typeof placeInfo.geocode === 'object') _this.marking(placeInfo.geocode , placeInfo, length, i, placeInfo.cnt);
				else {
					_this.geocoding(placeInfo.address, function(geocode, i) {
						_this.marking(geocode, placeInfo, length, i, placeInfo.cnt);
					});
				}
			}
			if(typeof data.placeAddress === 'string' || typeof data.placeName === 'string') {
				if(typeof data.placeAddress === 'string' && length !== 1) {
	 ...
	 */
	Geomap.prototype.marking = function(geocode, placeInfo, length, idx, cnt) {
		if(typeof this.opt.icon['type'] === 'string') {
			switch(this.opt.icon['type']) {
				case 'image':
					var content = {
						url: this.opt.icon['url']
					};
					break;
				case 'html':
					var content = {
						// content: '<p class="'+ this.opt.icon['className'] +'">'+ (idx + 1) +'</p>',
						content: '<p class="'+ this.opt.icon['className'] +'">'+ cnt +'</p>',
						size: new naver.maps.Size(this.opt.icon['size']),
						anchor: new naver.maps.Point(23, 35),
	...
	Geomap.prototype.openInfoWindow = function(marker, placeInfo) {
		...
		$.post(this.opt.infoWindow.requestUrl, placeInfo, function(response) {
            ...
            _this.infoWindowOptions['content'] = response;
			_this.infoWindow = new naver.maps.InfoWindow(_this.infoWindowOptions);
			_this.infoWindow.open(_this.map, marker);
			_this.map.panTo(marker.getPosition());
			marker.setAnimation(naver.maps.Animation.BOUNCE);
            $('.infoWindowClose').click(function(){
	...
	Geomap.prototype.deletePlaceMarkers = function() {
		if(typeof this.infoWindow == 'object') this.infoWindow.close();
	...
	Geomap.prototype.geocoding = function(address, callback) {
		naver.maps.Service.geocode({query: address}, function(status, response) {
			...
			if(status === naver.maps.Service.Status.OK && typeof result.addresses[0] === 'object') {
	...
	var Geomapping = function() {
		...
		this.initialize = function() {
			...
			gmap.initialize(container, options, function() {
				if(placeSearch === '') gmap.setPlaceMarkers(ajaxFilePath + '/place_find.php', {board: CONST_BOARD, sca: 'all'});
				else gmap.setPlaceMarkers(ajaxFilePath + '/place_find.php', {board: CONST_BOARD, sca: 'all', search: placeSearch, select: fieldList});
			});
		};
 ...
 ****************************************/
	...
	var GeomapFind = function() {
		...
		var listContainer = '.place_find_body',
			listScroller = '.place_find_result',
		...
		this.initialize = function() {
			...
			$(listScroller).mCustomScrollbar({
	            autoDraggerLength: false,
	            scrollInertia: 80
	        });
			$('body').on('click', '.infowindow-opener', function(e) {
				e.preventDefault();
				var query = {board: CONST_BOARD, sca: 'all', search: $(this).data('title'), select: 'title_01'};
		...
		this.local1Action = function(e) {
			...
			if(typeof depth2 === 'object') {
				...
				var address, search = '';
				if(local2Val === null) {
					address = local1Val.split('||')[0];
					search = local1Val;
				} else {
					if(local1Val.indexOf('||') > -1) {
						var tempSearch = local1Val.split('||');
						for(var i=0; i<tempSearch.length; i++) {
							if(i === 0) address = tempSearch[i]+' '+local2Val;
							if(i > 0) search += '||';
							search += tempSearch[i]+' '+local2Val;
						}
					} else {
						address = search = local1Val+' '+local2Val;
					}
				}
				var query = {board: CONST_BOARD, sca: 'all', search: search, select: 'addr_road||addr_number', placeAddress: address};
			} else {
				var query = {board: CONST_BOARD, sca: 'all'};
			}
			gmap.setPlaceMarkers(requestUrl, query);
		...
		this.local2Action = function(e) {
			...
			if(typeof local1Val === 'string' && local1Val !== local2Val) {
				var address, search = '';
				if(local1Val.indexOf('||') > -1) {
					var tempSearch = local1Val.split('||');
					for(var i=0; i<tempSearch.length; i++) {
		...
		this.findAction = function(e) {
			...
			var query = {board: CONST_BOARD, sca: 'all', search: value, select: 'title_01||addr_road||addr_number', placeName: value};
			gmap.setPlaceMarkers(requestUrl, query);
			_this.updateList(query);
			e.preventDefault();
		};
		...
		this.updateList = function(query) {
			var listQuery = typeof query.search === 'string' ? {board: CONST_BOARD, sca: 'all', search: query.search, select: query.select} : {board: CONST_BOARD, sca: 'all'};
			$.get(listUrl, listQuery, function(response) {
				$(listContainer).html($(response).find(listContainer).html());
				$(listScroller).mCustomScrollbar({
		            autoDraggerLength: false,
		            scrollInertia: 80
		        });
	...
	$(document).ready(function() {
		new Geomapping();
		new GeomapFind();
		new Tabbing($('.place_find_tabs'), $('.place_find_cons'));
		...
		(function() {
            ...
            function modalX(e) {
				e.preventDefault();
				TweenLite.to($container, 0.2, {display: 'none', opacity: 0, onComplete: function() {
					$container.empty();
                    open = false;
				}});
            }
			...
			function modal(idx) {
				if(open === true) $(_closer).trigger('click');
                if(changing === false) {
                    $.post(CONST_SKIN_PATH + '/resource/view.php', {skin_path: CONST_SKIN_PATH, idx: idx}, function(response) {
                        $container.append(response);
						TweenLite.fromTo($container, 0.4, {display: 'none', opacity: 0}, {display: 'block', opacity: 1, onComplete: function() {
                            callback();
                            changing = false;
                            open = true;
						}});
			...
			var $topElement = $('body'),
                $container = $('#placeModalContainer'),
                _opener = '.pv-opener',
                _closer = '.pv-closer',
				_scroller = '.pv-scroller',
                changing = false,
                open = false;

            $topElement.on('click', _opener, modalOpen);
            $topElement.on('click', _closer, modalX);

            function callback() {
				$(_scroller).mCustomScrollbar({
					scrollInertia: 80
				});
            }

			var _figure = '.place_pop_imgs .img';
			var _thumbs = '.place_pop_thumb li';
			$topElement.on('click', '.place_pop_thumb a', function(e) {
				$(_thumbs).filter('.on').removeClass('on');
				$(this).closest('li').addClass('on');
				e.preventDefault();
				$(_figure).css({backgroundImage: 'url(\''+ $(this).data('src') +'\')'});
			});

			window.modalO = function(idx) {
				modal(idx);
			};
        }());
	});

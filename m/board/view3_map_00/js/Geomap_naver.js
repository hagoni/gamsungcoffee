...
(function($) {
	...
	Geomap.prototype.setPlaceMarkers = function(requestUrl, data, callback) {
		...
		$.post(requestUrl, data, function(response) {
			...
			for(var i=0; i<length; i++) {
				...
				if(typeof placeInfo.geocode === 'object') _this.marking(placeInfo.geocode , placeInfo, length, i);
				else {
					_this.geocoding(placeInfo.address, function(geocode, i) {
						_this.marking(geocode, placeInfo, length, i);
					});
				}
			}
	 ...
	 */
	Geomap.prototype.marking = function(geocode, placeInfo, length, idx) {
	...
	var Geomapping = function() {
		...
		this.initialize = function() {
			...
			gmap.initialize(container, options, function() {
				if(param_search === '') gmap.setPlaceMarkers(ajaxFilePath + '/place_find.php', {board: CONST_BOARD, sca: 'all'});
